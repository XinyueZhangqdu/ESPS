...
class CVDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.edge:
            ...
            #print(type(torch.from_numpy(boundary) > 0))
            ...
            #plt.imshow(img1*255)
            #plt.show()
            #print(f'edge map 0304 {edgemap.shape}')
            '''
            plt.imshow(label)
            plt.show()
            plt.imshow(edgemap)
            plt.show()
            '''
            return (image, label, edgemap,filename)
        else:
