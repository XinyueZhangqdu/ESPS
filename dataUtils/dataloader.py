import os
import dataUtils.CVTransforms as cvTransforms
import dataUtils.PILTransform as pilTransforms

from torch.utils import data
from torchvision import datasets
import torchvision.transforms as transforms
import dataUtils.DataSet as myDataLoader
import torch
import dataUtils.loadData as ld
import pickle



def portrait_CVdataloader(cached_data_file, data_dir, classes, batch_size, scaleIn,
                           w=180, h=320, edge=False, num_work=4, Enc = True, Augset= False):
    print("data .dataloader")
    print("cached_data_file {} data_dir {}".format(cached_data_file,data_dir))
    ...
    '''
    trainLoader = torch.utils.data.DataLoader(
        myDataLoader.CVDataset(data['trainIm'], data['trainAnnot'], edge= edge, Enc=Enc),
        batch_size=batch_size, shuffle=True, num_workers=num_work, pin_memory=True)
    '''
